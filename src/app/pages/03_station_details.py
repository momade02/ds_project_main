...
def main() -> None:
    ...
    # If still none, provide selection fallback if we have ranked stations
    if station is None:
        st.warning("No station is currently selected.")
        if ranked:
            st.markdown("Select a station from the most recent route run:")
            labels = [_station_label(s, i) for i, s in enumerate(ranked[:20])]
            choice = st.selectbox("Stations", options=labels, index=0)
            chosen_idx = labels.index(choice)
            chosen_station = ranked[chosen_idx]
            chosen_uuid = _station_uuid(chosen_station) or ""

            if st.button("Open selected station", use_container_width=True):
            ...
            st.info("Run the recommender first (Trip Planner) or use the Station Explorer once available.")
        return
